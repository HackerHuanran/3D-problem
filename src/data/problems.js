...
const baseProblems = [
  // ─── 新手问题 ───
  {
    ...
    solutions: [
      { step: 1, title: '先重新调平热床', detail: '把喷嘴和热床都预热到平时打印的常用温度，回零后依次检查四角和中间。每个位置都放一张A4纸，调到纸张能被轻轻拖动但不会卡死。第一轮调完后再回到第一个角复查一遍，确保不是前面调完后把别的角带偏了。' },
      { step: 2, title: '把热床温度加到合适区间', detail: '如果你打 PLA，先把热床设到 60–70°C；PETG 通常 70–85°C；ABS / ASA 通常要 95–110°C。不要一下拉太高，建议先比当前值增加 5°C 做测试，看边角是否更稳。' },
      { step: 3, title: '在切片里开启 Brim 裙边', detail: '打开切片软件，搜索 Brim 或“裙边”，先从 5mm 宽度开始；底面积很小或细长模型可以直接开到 8–10mm。重新切片时确认 Brim 和模型底边真正连在一起，而不是只绕一圈不接触。' },
      { step: 4, title: '把第一层风扇先关掉或延后', detail: 'PLA 也不要急着第一层就全风，先把第一层风扇设为 0%，第二层再逐步恢复。打印 ABS / ASA 时通常应全程关闭部件冷却风扇，否则边角会因为降温太快更容易翘起来。' },
      { step: 5, title: '给热床补一点附着辅助', detail: '如果你用的是玻璃、光面 PEI 或已经有点老化的板子，可以薄薄涂一层胶棒作为辅助。重点是薄而均匀，不要糊成块；打完等热床冷却后再取件，很多模型会自己松开。' },
      { step: 6, title: '把第一层速度降下来', detail: '第一层打印速度建议先压到 20–30mm/s，外墙和小特征甚至可以更慢。首层目标不是快，而是稳稳压实并真正粘住热床，等首层稳定后再考虑恢复正常速度。' }
    ],
  ...
  },
  {
    ...
    solutions: [
      { step: 1, title: '先把回抽参数调到合理范围', detail: '如果你是直驱机型，先从 1–2mm 回抽距离试起；Bowden 机型先从 4–5mm 试起。回抽速度可从 25–40mm/s 起步，每次只改一个参数，并打印同一个拉丝测试件对比，不要距离和速度同时乱改。' },
      { step: 2, title: '把喷嘴温度一点点往下压', detail: '先记录当前温度，再每次降低 5°C 做同一模型测试。观察的不只是拉丝多少，还要看层间结合和表面是否变差；如果拉丝变少但层间开始发脆，就说明已经降过头了。' },
      { step: 3, title: '提高空移速度减少漏料时间', detail: '在切片软件里找到 Travel Speed 或空驶速度，先提高到 150–200mm/s 区间。空移越快，喷嘴停留在空气中的时间越短，拉丝机会也会随之下降。' },
      { step: 4, title: '启用走线路径优化', detail: '如果切片器支持 Combing、Avoid crossing walls 或类似路径优化选项，优先让喷嘴在已打印区域内部移动。这样即使有少量渗料，也更容易藏在内部，不会挂在外表面。' },
      { step: 5, title: '排除耗材受潮这个隐藏问题', detail: '如果拉丝很多并且伴随“噼啪”爆裂声、表面毛糙或小气泡，优先怀疑受潮。把耗材按材质要求烘干后再用同一份切片复打一遍，如果明显改善，就别再继续死调回抽了。' }
    ],
  ...
  },
  {
    ...
    solutions: [
      { step: 1, title: '先排除最简单的供料问题', detail: '先看线盘里是否还有料，再顺着耗材路径从线盘一路看到进料口，确认没有打结、卡边或被线盘边缘夹住。很多“完全不出丝”其实不是喷嘴坏了，而是前面根本送不过来。' },
      { step: 2, title: '观察送丝器到底有没有咬住耗材', detail: '预热后尝试手动挤出，同时看挤出机齿轮是否在转、耗材是否在前进。如果齿轮空转、磨粉或只在原地打滑，说明耗材没有被正常抓住，需要重新装料、调整压紧力，或剪掉被磨坏的那一段。' },
      { step: 3, title: '确认喷嘴温度真的达到可打印区间', detail: '不要只看目标温度，确认屏幕上的实际温度已经稳定达到设定值。PLA 通常要 195–220°C 左右，PETG 常在 230°C 左右，ABS 更高；温度太低时，齿轮会推不动，表现就像堵头。' },
      { step: 4, title: '检查是不是首层压得太死', detail: '如果一开始出了一点丝，喷嘴一靠近热床就没了，优先怀疑 Z 偏移过低。进入机器的 Baby Step 或 Z-offset 调整界面，把喷嘴每次抬高 0.02–0.05mm，再看首层是否恢复正常出料。' },
      { step: 5, title: '做喷嘴清理或直接更换喷嘴', detail: '如果手动推料还是推不动，就按冷拔或热拔流程清一次喷嘴。老喷嘴内部有积碳、杂质或被磨损变形时，直接换一个新喷嘴通常比硬拖着调参数更快更稳。' }
    ],
  ...
  },
  {
    ...
    solutions: [
      { step: 1, title: '先把热床彻底清洁干净', detail: '打印前用异丙醇把热床整面擦一遍，重点擦掉手油、灰尘和旧残胶。擦完后尽量不要再用手直接摸打印区域，否则刚清干净的表面又会马上沾上油脂。' },
  ...
  {
    id: 'partial-bed-adhesion',
    video: null,
    category: '新手',
    printerType: 'FDM',
    title: '局部不粘床',
    subtitle: '只有一角或一侧反复翘起',
    emoji: '📍',
    color: '#FF8A65',
    bgGradient: 'linear-gradient(135deg, #1a0d08 0%, #2d160f 100%)',
    difficulty: '常见',
    description: '模型不是整体都粘不住，而是固定某个角、某一侧或热床某个区域总是先失败。这通常不是简单的“温度不够”，而是热床局部状态或风场不均导致。',
    causes: ['热床局部高低不平', '该区域表面有油污或旧胶残留', '风扇或空调直吹某一边', '模型摆放位置刚好落在温差区'],
    solutions: [
      { step: 1, title: '做首层网格测试', detail: '打印大面积首层网格，观察失败区域是否固定在热床某一角。如果总在同一位置出问题，优先怀疑局部调平或床面状态，而不是整机参数。' },
      { step: 2, title: '重点清洁问题区域', detail: '用异丙醇反复擦拭出问题的区域，玻璃床可重新薄涂胶棒，PEI板可用温水加少量洗洁精清洗后彻底晾干。' },
      { step: 3, title: '移动模型位置验证', detail: '将同一模型平移到热床另一侧再打一次。如果失败位置跟着热床走，说明是床面/风场问题；如果跟着模型走，说明是模型底面或切片问题。' },
      { step: 4, title: '检查环境风向', detail: '开放式打印机旁边的空调、风扇、窗户会让某一侧更快冷却。打印首层和前几层时尽量避免侧吹，必要时加挡风板。' }
    ],
    tips: '“总是某个角失败”这类问题最怕凭感觉乱调整体温度。先确认失败是跟着“位置”走还是跟着“模型”走，能少走很多弯路。'
  },
  {
    id: 'first-layer-over-squished',
    video: null,
    category: '新手',
    printerType: 'FDM',
    title: '首层过度挤压',
    subtitle: '第一层像被抹开，边缘发亮',
    emoji: '🧈',
    color: '#FFD166',
    bgGradient: 'linear-gradient(135deg, #1a1404 0%, #2d2308 100%)',
    difficulty: '常见',
    description: '第一层虽然能粘住，但线条被严重压扁、边缘鼓起发亮，后续容易出现象脚、尺寸偏大和表面发毛。',
    causes: ['Z-offset过低', '第一层流量过高', '首层层高过小', '热床过热导致底层长时间过软'],
    solutions: [
      { step: 1, title: '微抬Z偏移', detail: '打印首层测试时，每次将Z-offset抬高0.02–0.05mm，直到线条仍能连成面但不再出现明显堆边和发亮。' },
      { step: 2, title: '降低第一层Flow', detail: '将第一层挤出倍率从100%降到95%甚至92%，减少底层被横向挤开的程度。' },
      { step: 3, title: '检查首层层高', detail: '0.4mm喷嘴建议首层层高设在0.20–0.28mm之间。首层太薄时，即使Z偏移正确，也更容易显得“过压”。' },
      { step: 4, title: '适度降低床温', detail: 'PLA首层若总是软塌发亮，可把热床温度从65°C降到55–60°C试试，避免底层长时间处于过软状态。' }
    ],
    tips: '“能粘住”不等于“首层正确”。好的首层应该是线条轻微压扁并彼此贴合，而不是像抹黄油一样被挤成一整片。'
  },
  {
    id: 'nozzle-scraping-print',
    video: null,
    category: '进阶',
    printerType: 'FDM',
    title: '喷嘴刮擦模型',
    subtitle: '打印时反复蹭件或发出咔咔声',
    emoji: '🪒',
    color: '#B388FF',
    bgGradient: 'linear-gradient(135deg, #12061a 0%, #1f0c2d 100%)',
    difficulty: '需处理',
    description: '打印中喷嘴持续擦碰已经打印好的表面，轻则留下划痕和粗糙层纹，重则撞掉模型导致层移或整件失败。',
    causes: ['过挤出导致层层堆高', 'Z-Hop未开启或高度不足', '局部翘曲抬起', '旅行路径穿越模型顶部'],
    solutions: [
      { step: 1, title: '先排除过挤出', detail: '如果顶面越来越鼓，优先检查Flow Rate、E-step和线宽设置。持续轻微过挤出会让实际高度不断累积，最终导致喷嘴刮件。' },
      { step: 2, title: '开启或增大Z-Hop', detail: '在切片软件中开启Z-Hop（抬头）0.2–0.4mm，让空移时喷嘴抬起，避免穿过已经成型的高点。' },
      { step: 3, title: '检查模型是否局部翘起', detail: 'ABS、大平面件和薄壁件在中途容易边缘上翘。若只有某一段开始刮件，重点检查那里是否已经变形抬起。' },
      { step: 4, title: '优化Travel路径', detail: '将空走策略改为“避开外墙/避开顶部表面”，减少喷嘴直接跨越可见表面的次数。' }
    ],
    tips: '如果刮件声音只在空走时出现，重点看Z-Hop和Travel；如果连正常挤出时也在刮，优先怀疑过挤出或模型已变形。'
  },
  {
    id: 'intermittent-under-extrusion',
    video: null,
    category: '进阶',
    printerType: 'FDM',
    title: '间歇性欠挤出',
    subtitle: '某些层突然发虚，之后又恢复',
    emoji: '📉',
    color: '#64B5F6',
    bgGradient: 'linear-gradient(135deg, #08121a 0%, #0f2230 100%)',
    difficulty: '需处理',
    description: '打印并非持续欠挤出，而是某些层、某些区域突然出丝变少，随后又恢复正常。这类问题通常比完全堵头更难查。',
    causes: ['喷嘴半堵塞', '热端散热不足导致热蠕变', '线盘阻力时大时小', '挤出机压紧力不稳定'],
    solutions: [
      { step: 1, title: '检查线盘放料阻力', detail: '手动拉动耗材，确认整卷放料顺畅，没有在边缘卡顿、打结或摩擦过大。很多“间歇性欠挤出”其实是供料端偶发阻力。' },
      { step: 2, title: '做半堵塞清理', detail: '先做冷拔，再观察喷嘴是否有明显偏喷、歪喷和吐丝不圆。半堵塞会在高流量段或长空移后更明显。' },
      { step: 3, title: '检查热端风扇与散热片', detail: '热端散热风扇转速不足时，耗材会在热断区提前变软，长时间打印后更容易出现时好时坏的挤出。' },
      { step: 4, title: '复核挤出机压紧力', detail: '弹簧太松会打滑，太紧会磨伤耗材。观察耗材表面齿痕，如果被磨出很深的槽，说明压得太狠。' }
    ],
    tips: '这类问题最有价值的线索是“它在什么时候发作”，例如高速填充时、长时间打印后、换层后或回抽多的时候。'
  },
  {
    id: 'corner-bulging',
    video: null,
    ...
    emoji: '📦',
    color: '#FFB74D',
    bgGradient: 'linear-gradient(135deg, #1a1006 0%, #2d1c0a 100%)',
    difficulty: '进阶',
    description: '打印方形或带锐角的模型时，转角处会明显堆料、发圆或鼓出来，导致边缘不利落，尺寸和外观都受影响。',
    causes: ['拐角减速时压力释放不及时', 'Pressure Advance未校准', '外墙速度过低', '温度偏高导致渗料'],
    solutions: [
      { step: 1, title: '校准Pressure Advance', detail: 'Klipper、Bambu Studio和OrcaSlicer都支持压力补偿。通过测试塔找到合适值，能明显改善转角堆料。' },
      { step: 2, title: '适度提高外墙速度', detail: '太慢会让喷嘴在转角停留更久，更容易多挤一点。可把外墙速度从30mm/s提高到40–50mm/s观察差异。' },
      { step: 3, title: '小幅降低打印温度', detail: '温度过高时熔体更容易在减速时往外鼓。每次降5°C测试，尤其对PETG和高流量PLA常有效。' },
      { step: 4, title: '启用外轮廓优化', detail: '部分切片器有“减少转角速度变化”或“改进外壁路径”选项，能缓解角点压力堆积。' }
    ],
    tips: '如果圆弧好看、只有直角难看，基本就不是机械精度问题，而更偏向挤出动态控制和切片策略。'
  },
  {
    id: 'hole-too-small',
    video: null,
    ...
    printerType: 'FDM',
    title: '孔径偏小',
    subtitle: '设计能插，打印后插不进去',
    emoji: '🕳️',
    color: '#4DB6AC',
    bgGradient: 'linear-gradient(135deg, #061815 0%, #0c2a24 100%)',
    difficulty: '常见',
    description: '打印件的圆孔、螺丝孔或轴承孔明显比设计值更小，导致插销、螺丝、轴承装不进去。',
    causes: ['FDM圆孔天然趋向内缩', '外轮廓过挤出', 'XY补偿未设置', '孔位朝向不合理导致悬垂变形'],
    solutions: [
      { step: 1, title: '先做Flow和E-step校准', detail: '如果整体都偏胖，先别急着调孔补偿。先保证挤出量准确，否则任何尺寸补偿都是建立在错误基础上。' },
      { step: 2, title: '使用XY/孔补偿', detail: '在切片器中启用水平扩展、孔补偿或XY尺寸补偿，先从0.05–0.15mm试起，逐步逼近目标尺寸。' },
      { step: 3, title: '关键孔位横放慎用', detail: '水平打印的圆孔顶部会形成桥接，实际更容易变小。若是精密孔，考虑改朝向或预留后加工余量。' },
      { step: 4, title: '设计阶段预留余量', detail: '普通插销孔通常要比名义尺寸放大0.15–0.30mm；轴承位和卡扣位则应按实际机型测试后定规则。' }
    ],
    tips: '“整体尺寸准，但孔总是小”是FDM很典型的特性，不一定是你机器坏了，更多时候需要单独为孔做补偿。'
  },
  {
    id: 'clearance-fit-tight',
    video: null,
    category: '切片',
    printerType: 'FDM',
    title: '活动件配合过紧',
    subtitle: '铰链、滑块、卡扣打印后卡死',
    emoji: '🧩',
    color: '#90A4AE',
    bgGradient: 'linear-gradient(135deg, #0d1215 0%, #172127 100%)',
    difficulty: '进阶',
    description: '模型在设计时本来是活动连接或滑动配合，但打印后根本动不了，或者需要非常大力才能掰开。',
    causes: ['预留间隙过小', '首层或外轮廓膨胀', '桥接和小特征精度不足', '材料收缩和机型误差未被考虑'],
    solutions: [
      { step: 1, title: '校准可动间隙测试件', detail: '打印0.1/0.2/0.3/0.4mm间隙测试件，找出你的机型在常用材料下真正可活动的最小间隙值。' },
      { step: 2, title: '关键配合面减少象脚影响', detail: '活动结构通常最怕底部膨胀。可开启象脚补偿、抬高Z偏移或把活动面从首层区域移开。' },
      { step: 3, title: '为活动件单独放大间隙', detail: '普通PLA打印的打印一体铰链，建议至少预留0.25–0.35mm；PETG常需更大一些。' },
      { step: 4, title: '降低外墙过挤和温度', detail: '外轮廓偏胖会直接吃掉活动间隙。若活动件总卡死，先排查外墙尺寸是否普遍偏大。' }
    ],
    tips: '活动件问题最怕“按理论尺寸设计”。3D打印的可动间隙一定要按材料、喷嘴、层高和机器状态建立自己的经验值。'
  },
  {
    id: 'tall-print-wobble',
    video: null,
    category: '进阶',
    printerType: 'FDM',
    title: '高件打印晃动',
    subtitle: '模型越往上越抖，表面发虚',
    emoji: '🗼',
    color: '#9575CD',
    bgGradient: 'linear-gradient(135deg, #100a18 0%, #1c1230 100%)',
    difficulty: '需处理',
    description: '细高模型在打印到中后期时明显晃动，喷嘴一碰就抖，最终导致表面粗糙、尺寸变差，甚至整件倒掉。',
    causes: ['模型底面积太小', '打印速度和加速度过高', '冷却不足导致高层太软', 'Z-Hop和Travel路径碰撞模型'],
    solutions: [
      { step: 1, title: '减速并降低加速度', detail: '把外墙和小层打印速度降下来，同时把加速度从3000降到1000–1500mm/s²，能显著降低顶部摆动。' },
      { step: 2, title: '加Brim或辅助支撑', detail: '细高件即使首层粘住，也可能因为杠杆效应晃动。加宽Brim、耳朵（mouse ear）或临时侧支撑能稳定底部。' },
      { step: 3, title: '延长最短层时间', detail: '小截面高件每层时间太短，上一层还没冷却好下一层又压上去，顶部更容易被带着晃。可开启最短层时间或降低小层速度。' },
      { step: 4, title: '优化打印顺序', detail: '如果同盘有多个高件，可让打印机轮流打印不同件，给单个模型更多冷却时间。' }
    ],
    tips: '高件失败很多时候不是“粘床问题”，而是动力学问题。底下稳不代表上面稳，越高越要重视速度、冷却和路径碰撞。'
  },
  {
    id: 'petg-bed-too-sticky',
    video: null,
    category: '材料',
    printerType: 'FDM',
    title: 'PETG粘床太牢',
    ...
    emoji: '🪵',
    color: '#26A69A',
    bgGradient: 'linear-gradient(135deg, #061816 0%, #0b2b25 100%)',
    difficulty: '常见',
    description: 'PETG在某些床面上附着力极强，打印后即使冷却也难以取下，严重时会把玻璃表面或涂层直接带走。',
    causes: ['PETG与光面PEI或玻璃直接强粘', '第一层压得过低', '床温过高', '未使用隔离层'],
    solutions: [
      { step: 1, title: '使用隔离层', detail: '在玻璃或光面PEI上打印PETG时，可薄涂胶棒或喷少量发胶，不是为了更粘，而是为了形成一层可释放的隔离膜。' },
      { step: 2, title: '不要把首层压太死', detail: 'PETG不需要像PLA那样压得很扁。适当抬高Z-offset，让首层成型即可，避免直接焊死在床上。' },
      { step: 3, title: '等待完全冷却再取件', detail: 'PETG通常在热的时候最难取。等热床降到30°C以下，很多模型会明显松动，再借助柔性钢板弯折取件。' },
      { step: 4, title: '改用更适合的床面', detail: '纹理PEI、粉末喷涂板或专用释放片通常比玻璃更适合PETG，既能粘住又不至于粘得过头。' }
    ],
    tips: 'PETG的“粘床”问题常常不是不够粘，而是太粘。尤其在玻璃上，直接硬撬非常容易把床面一起带下来。'
  },
  {
    id: 'tpu-buckling',
    video: null,
    ...
    printerType: 'FDM',
    title: 'TPU送丝打弯',
    subtitle: '耗材在挤出机前鼓包乱跑',
    emoji: '🪢',
    color: '#4FC3F7',
    bgGradient: 'linear-gradient(135deg, #06131a 0%, #0b2430 100%)',
    difficulty: '需处理',
    description: '打印TPU时，耗材还没进入热端就先在挤出机附近打弯、鼓包或从导料口挤出来，导致完全无法稳定打印。',
    causes: ['挤出路径间隙大', '回抽过大过快', '送料速度过快', '耗材太软且受潮'],
    solutions: [
      { step: 1, title: '尽量使用直驱并缩短路径', detail: 'TPU最怕长距离、宽缝隙的料路。直驱挤出机比Bowden更适合；若是Bowden机型，尽量使用高硬度TPU并降低速度。' },
      { step: 2, title: '大幅减小回抽', detail: 'TPU回抽通常只需0.5–1.5mm，甚至可以关闭回抽。回抽过大时，软料最容易在进料路径里被拉弯。' },
      { step: 3, title: '降低打印和送料速度', detail: '将打印速度降到20–35mm/s，避免挤出机突然大力推拉。柔性材料靠稳定、慢速、连续送料来成型。' },
      { step: 4, title: '确认耗材干燥并压紧适中', detail: '受潮TPU更软、更粘，也更容易打弯。挤出机压紧力不能过大，否则会把材料挤扁。' }
    ],
    tips: 'TPU能不能打好，很多时候不是切片参数决定的，而是挤出路径机械结构本身决定的。料路越短、间隙越小，成功率越高。'
  },
  {
    id: 'nozzle-leak',
    video: null,
    ...
    printerType: 'FDM',
    title: '热端漏料',
    subtitle: '加热块周围挂满焦黑残料',
    emoji: '🫗',
    color: '#8D6E63',
    bgGradient: 'linear-gradient(135deg, #140d0a 0%, #241712 100%)',
    difficulty: '需处理',
    description: '耗材不是只从喷嘴口出来，而是从喷嘴与热断、加热块连接处渗出，时间久了会在加热块外面结成一团焦黑残料。',
    causes: ['喷嘴热紧不到位', '热断与喷嘴未顶死', '拆装时未在工作温度下锁紧', '密封面有残渣'],
    solutions: [
      { step: 1, title: '热态重新锁紧喷嘴', detail: '将热端加热到工作温度后，再按规范扭矩轻轻锁紧喷嘴。冷态锁紧常会在热胀后留下缝隙。' },
      { step: 2, title: '确认喷嘴与热断顶死', detail: '真正密封的位置是喷嘴尾端与热断前端的贴合，而不是喷嘴六角完全贴住加热块。装配时要确保内部接触面完全密封。' },
      { step: 3, title: '清理残料再复装', detail: '漏料后先彻底清掉螺纹和密封面上的旧料与积碳，再重新装配，否则残渣会妨碍密封。' },
      { step: 4, title: '检查硅胶套与热块状态', detail: '如果漏料已经积成大块，顺手检查硅胶套是否老化、热敏电阻和加热棒线材是否被污染或拉扯。' }
    ],
    tips: '“喷嘴拧到底就行”是常见误区。热端密封靠的是内部端面贴合，不是外面六角螺帽看起来有多紧。'
  },
  {
    id: 'z-axis-binding',
    video: null,
    ...
    printerType: 'FDM',
    title: 'Z轴卡滞',
    subtitle: '升降不顺，某些高度特别难走',
    emoji: '🪜',
    color: '#78909C',
    bgGradient: 'linear-gradient(135deg, #0d1215 0%, #1a242b 100%)',
    difficulty: '需处理',
    description: 'Z轴在某些高度移动不顺畅，打印时会出现层高忽大忽小、局部堆料、换层异响，严重时还会直接丢层。',
    causes: ['丝杆脏污或缺润滑', '联轴器偏心', '双Z不同步', '导轨或轮组预紧过大'],
    solutions: [
      { step: 1, title: '手动空载检查阻力', detail: '断电后手动转动丝杆或抬升横梁，感受是否在固定高度有明显阻力变化。如果总在同一段变紧，优先查机械干涉。' },
      { step: 2, title: '清洁并润滑丝杆', detail: '用无纺布擦去旧油和粉尘，再补少量适合丝杆的润滑脂。太干或太脏都会导致运动不均匀。' },
      { step: 3, title: '检查双Z同步和联轴器', detail: '双Z结构若左右不同高，会让横梁上升时一边拖拽另一边。联轴器若压得过死或装歪，也会把偏摆传给丝杆。' },
      { step: 4, title: '复核导向件预紧力', detail: 'V轮太紧、线轨安装不平或滑块阻力异常，都会让Z轴看似是丝杆问题，实则是导向阻力过大。' }
    ],
    tips: '很多人把所有竖纹都归因于“丝杆弯”。其实更常见的是脏、干、不同步或导向阻力异常，先从这些基础项查起。'
  },
  {
    id: 'resin-support-failure',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '光固化支撑中途断裂',
    subtitle: '底座正常，模型半路掉了',
    emoji: '🪤',
    color: '#AB47BC',
    bgGradient: 'linear-gradient(135deg, #14061a 0%, #240b2d 100%)',
    difficulty: '需处理',
    description: '平台和底座都正常成型，但模型主体在打印中途从支撑上脱落，最后只剩支撑森林或半截模型。',
    causes: ['支撑太细或密度不足', '模型角度不合理导致受力集中', '曝光不足导致支撑强度不够', '离型力过大'],
    solutions: [
      { step: 1, title: '先加粗主支撑', detail: '对承重大、最先接触模型的主支撑提高直径和接触点尺寸，不要只是一味加数量。关键受力点要足够强。' },
      { step: 2, title: '调整模型倾斜角度', detail: '避免大截面突然出现，把模型倾斜15–35°，让每层截面积变化更平滑，减轻支撑受力冲击。' },
      { step: 3, title: '小幅提高普通层曝光', detail: '如果支撑本身发软、容易断，可把普通层曝光增加0.1–0.3秒验证，但不要一步加太多以免细节糊掉。' },
      { step: 4, title: '复查离型参数和膜状态', detail: '抬升过快、FEP过旧或吸盘效应明显时，哪怕支撑数量够，也可能被整层剥断。' }
    ],
    tips: '光固化支撑失败最常见的误区是“多加支撑就行”。真正关键的是主支撑位置、角度和每层受力分布。'
  },
  {
    id: 'resin-detail-loss',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '光固化细节发糊',
    subtitle: '小字、纹理、尖角不清晰',
    emoji: '🔬',
    color: '#5C6BC0',
    bgGradient: 'linear-gradient(135deg, #0b0f1a 0%, #131d30 100%)',
    difficulty: '常见',
    description: '光固化打印整体成型没问题，但小字、微小凹槽、锐边和细纹理都发糊，缺少应有的锐利度。',
    causes: ['曝光时间偏长', '层高过大', '树脂老化或颜料分层', '清洗和二固过度'],
    solutions: [
      { step: 1, title: '先做曝光测试片', detail: '不要直接凭感觉减曝光。用官方或社区常见曝光测试模型找到细节与成型成功率之间的平衡点。' },
      { step: 2, title: '降低层高提升细节', detail: '从0.05mm改到0.03mm，通常能明显改善小字和浅纹理表现，尤其适合手办、模型和微小结构。' },
      { step: 3, title: '充分摇匀并过滤树脂', detail: '长期放置的树脂颜料会沉降，导致局部曝光特性不稳定。打印前摇匀，必要时过滤掉杂质和半固化颗粒。' },
      { step: 4, title: '控制后处理强度', detail: '清洗太久会侵蚀细小凸起，二次固化太猛会让边缘变钝。细节件的后处理应比普通功能件更温和。' }
    ],
    tips: '细节发糊很多时候并不是机器分辨率不够，而是曝光和后处理把“本来能显示出来的细节”给吃掉了。'
  },
  {
    id: 'resin-floating-islands',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '光固化漂浮层/孤岛',
    subtitle: '槽里有碎片，模型局部缺失',
    emoji: '🏝️',
    color: '#7E57C2',
    bgGradient: 'linear-gradient(135deg, #10071a 0%, #1b0d2f 100%)',
    difficulty: '需处理',
    description: '切片中某些区域没有被支撑到，打印时会在树脂槽里形成独立固化碎片，既导致模型缺失，也可能划伤FEP。',
    causes: ['模型存在孤岛未被发现', '自动支撑漏掉关键悬空区', '中空内壁或小凸台缺支撑', '切片预览未逐层检查'],
    solutions: [
      { step: 1, title: '逐层检查切片预览', detail: '尤其看最开始出现实体的那几层，任何“凭空出现”的截面都必须被支撑到。自动支撑不能完全替代人工审查。' },
      { step: 2, title: '对小悬空特征单独补支撑', detail: '耳朵、指尖、小凸点、内腔边缘这些地方最容易漏支撑，需要手动补轻支撑或改角度。' },
      { step: 3, title: '失败后清槽过滤树脂', detail: '一旦怀疑有漂浮碎片，必须停机过滤树脂并检查FEP，别直接继续打印，否则碎片可能被压在膜上造成破损。' },
      { step: 4, title: '合理倾斜减少孤岛出现', detail: '把模型稍微倾斜后，很多原本突然出现的大面积孤岛会变成逐层渐进出现，更容易被稳定支撑。' }
    ],
    tips: '树脂打印最值钱的习惯之一，就是正式开打前先花30秒逐层拖一下预览，看有没有任何“突然冒出来”的区域。'
  },
  {
    id: 'resin-hollow-cracking',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '中空件后期开裂',
    subtitle: '打印后几天自己炸裂渗液',
    emoji: '💥',
    color: '#EF5350',
    bgGradient: 'linear-gradient(135deg, #1a0808 0%, #2d1010 100%)',
    difficulty: '紧急',
    description: '模型刚打印出来看着正常，过几天却沿着薄弱处开裂，甚至渗出树脂。这通常发生在中空模型内部残留未固化树脂时。',
    causes: ['中空腔体没有排液孔', '内部树脂未清洗干净', '内壁未二次固化', '壁厚过薄承受内应力差'],
    solutions: [
      { step: 1, title: '设计排液孔和通气孔', detail: '中空模型至少要有两个有效孔位，便于酒精进入和树脂排出。孔径过小不仅难排液，也难真正清洗到内壁。' },
      { step: 2, title: '延长内部清洗流程', detail: '清洗时要让酒精进入内腔反复冲洗，必要时用注射器或冲洗瓶辅助，而不是只洗外表面。' },
      { step: 3, title: '确保内部也被固化', detail: '有条件可通过孔位照射内部，或在透明树脂情况下用更均匀的多角度固化方式，减少内壁残留活性树脂。' },
      { step: 4, title: '功能件适当加厚壁厚', detail: '过薄的壳体更怕内部残留带来的应力。展示件和容器类模型通常建议壁厚不要低于2mm。' }
    ],
    tips: '中空件“后炸”几乎总和内部残液有关。只要还有未处理树脂被封在里面，时间久了就像把定时炸弹留在模型内部。'
  },
  {
    id: 'resin-white-frost',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '光固化表面发白',
    ...
    emoji: '🌫️',
    color: '#B0BEC5',
    bgGradient: 'linear-gradient(135deg, #0f1417 0%, #1a2328 100%)',
    difficulty: '常见',
    description: '模型原本打印出来挺通透或颜色正常，但清洗和固化后表面发白、发灰，像蒙了一层雾，影响成品观感。',
    causes: ['酒精未挥发就直接固化', '酒精浓度不够或已污染', '清洗后表面残留树脂皂化', '固化温度过高或时间过长'],
    solutions: [
      { step: 1, title: '清洗后彻底晾干', detail: '清洗结束后先自然风干或用压缩空气吹干，确认模型表面没有残留酒精和液滴，再进入固化步骤。' },
      { step: 2, title: '更换干净高浓度酒精', detail: '脏酒精会把溶解下来的树脂重新抹回模型表面，固化后就容易形成白雾层。' },
      { step: 3, title: '缩短固化时间分段观察', detail: '不要一次性猛固化10分钟。细节件和透明件建议短时分段固化，边照边看效果。' },
      { step: 4, title: '必要时做表面修复', detail: '轻微发白可用软布蘸少量新鲜酒精快速擦拭后重新轻固化，展示件也可通过上清漆恢复观感。' }
    ],
    tips: '树脂件发白常常发生在“打印其实没问题，后处理出了问题”这一阶段，所以别把所有锅都甩给曝光参数。'
  },
  {
    id: 'bridging-sagging',
    video: null,
    ...
    printerType: 'FDM',
    title: '桥接中间下坠',
    subtitle: '跨空部分像肚皮一样塌下来',
    emoji: '🌉',
    color: '#42A5F5',
    bgGradient: 'linear-gradient(135deg, #06111a 0%, #0c2030 100%)',
    difficulty: '常见',
    description: '模型需要跨越空隙时，桥接线条无法保持拉直，在中间明显下垂，造成底面粗糙、孔洞或直接坍塌。',
    causes: ['桥接风扇不足', '桥接速度过慢导致熔料来不及拉直', '桥接流量过高', '跨度超过材料和机型极限'],
    solutions: [
      { step: 1, title: '单独调桥接参数', detail: '在切片软件中开启桥接专用参数，优先提高桥接风扇、降低桥接流量到90–95%，并使用专用桥接速度。' },
      { step: 2, title: '不要一味降速', detail: '桥接并不是越慢越好。很多机型在25–40mm/s时反而更容易把料“拉过去”，太慢会直接往下坠。' },
      { step: 3, title: '适度降温', detail: '温度高时桥接线条更软。每次降低5°C试打桥接测试件，观察中间下垂是否改善。' },
      { step: 4, title: '优化模型朝向', detail: '能旋转就旋转，尽量让长桥接变短，或者改成需要支撑的方向，别硬让打印机跨超出能力范围的跨度。' }
    ],
    tips: '桥接问题一定要用桥接测试件单独测，不要拿普通模型猜。不同材料、风道和喷嘴状态，桥接极限差别会非常大。'
  },
  {
    id: 'small-part-overheating',
    video: null,
    category: '切片',
    printerType: 'FDM',
    title: '小零件过热变形',
    subtitle: '小尖塔、小柱子顶部发软发糊',
    emoji: '🕯️',
    color: '#FF7043',
    bgGradient: 'linear-gradient(135deg, #1a0d08 0%, #2d160f 100%)',
    difficulty: '常见',
    description: '打印小尺寸零件或高而细的小柱时，上层还没冷却就被下一层继续覆盖，导致顶端发软、卷边、糊成一团。',
    causes: ['单层打印时间太短', '风扇风量不足', '喷嘴在局部停留过久', '同时打印对象太少'],
    solutions: [
      { step: 1, title: '设置最短层时间', detail: '在切片软件中开启“最短层时间”，让打印机在每层太快结束时自动降速，给上一层更多冷却时间。' },
      { step: 2, title: '增加同盘打印数量', detail: '如果模型允许，可同时放2到4个相同小件，让喷嘴轮流打印，每个件都有额外冷却时间。' },
      { step: 3, title: '加强部件冷却', detail: '检查风道是否堵塞、风扇是否转速正常。PLA打小件时通常需要较强冷却，PETG则要在不过度吹塌的前提下平衡。' },
      { step: 4, title: '降低喷嘴温度', detail: '小件发糊时，适度降温通常比一味降速更有效。先降低5°C测试，减少熔料在顶部堆软。' }
    ],
    tips: '小件失败很多时候不是“机器不准”，而是“上一层还没凉”。只要层时间太短，再准的机器也会把顶端糊掉。'
  },
  {
    id: 'seam-gap',
    video: null,
    ...
    printerType: 'FDM',
    title: '接缝处有小坑',
    subtitle: '每层起始点像被咬了一口',
    emoji: '🪡',
    color: '#EC407A',
    bgGradient: 'linear-gradient(135deg, #1a0710 0%, #2d0d1a 100%)',
    difficulty: '进阶',
    description: '模型每层接缝处不是鼓包，而是出现细小凹坑、缺口或起点空白，垂直排列后会形成一条明显的“麻点缝”。',
    causes: ['回抽恢复不足', '额外回抽/擦拭设置不当', '线性提前过强', '起始点压力建立慢'],
    solutions: [
      { step: 1, title: '先减轻过度回抽', detail: '如果接缝处总是空一下再出料，先略微减小回抽距离或速度，避免喷嘴刚到起点时内部压力还没恢复。' },
      { step: 2, title: '调整Wipe和Coast', detail: '擦拭、停挤出提前等参数若开得太猛，会把尾端抽得太干，导致下一层起点缺料。' },
      { step: 3, title: '复查压力补偿值', detail: 'Pressure Advance/Linear Advance数值过高时，角落和起始点特别容易出现空缺。把值回退一点再打立柱测试件观察。' },
      { step: 4, title: '更换接缝位置策略', detail: '如果凹坑无法完全消除，可把接缝放在背面、锐边或不显眼区域，先把视觉影响降下来。' }
    ],
    tips: '接缝鼓包和接缝缺口是两种相反问题，别用同一套思路去调。鼓包常是压力太多，缺口常是压力建立太慢。'
  },
  {
    id: 'wall-delamination-corners',
    video: null,
    category: '进阶',
    printerType: 'FDM',
    title: '外墙拐角开缝',
    ...
    emoji: '📐',
    color: '#7E57C2',
    bgGradient: 'linear-gradient(135deg, #10071a 0%, #1c0d2d 100%)',
    difficulty: '需处理',
    description: '长直墙体还算完整，但拐角或尖角位置先出现细缝，随后沿着角落逐层扩大，最终形成明显开裂。',
    causes: ['角落冷却过快', '材料收缩应力集中', '墙体太薄', '外墙温度不足'],
    solutions: [
      { step: 1, title: '提高喷嘴温度一点点', detail: '如果只有角落层间开裂，可先把喷嘴温度提高5°C，让角点在减速转弯时也能保持良好层间融合。' },
      { step: 2, title: '减少风扇或环境直吹', detail: '尖角最容易被风先带走热量，尤其ABS、ASA和大件PETG。必要时降低风扇或增加挡风环境。' },
      { step: 3, title: '增加壁厚和圆角过渡', detail: '设计上让尖锐内角变圆角、让薄壁略厚一点，能明显降低角部收缩应力集中。' },
      { step: 4, title: '降低外墙速度', detail: '角落减速时虽然停留更久，但若速度整体太快、来不及粘合，也会让角部成为最先失效的位置。' }
    ],
    tips: '“只有拐角裂”通常不是单纯的整体欠挤出，而是角落处的热管理和收缩应力出了问题。'
  },
  {
    id: 'overhang-curling',
    video: null,
    ...
    printerType: 'FDM',
    title: '悬垂边缘上翘',
    subtitle: '斜面边缘卷起来，后面被喷嘴撞到',
    emoji: '🪶',
    color: '#26C6DA',
    bgGradient: 'linear-gradient(135deg, #061519 0%, #0b252d 100%)',
    difficulty: '需处理',
    description: '悬垂区域本来还能勉强打印，但边缘一层层往上卷，最后被喷嘴反复刮到，表面越来越糟，甚至导致整件失败。',
    causes: ['悬垂区域冷却不足', '温度偏高材料太软', '层时间过短', '悬垂角度接近材料极限'],
    solutions: [
      { step: 1, title: '加强悬垂区域冷却', detail: '对于PLA等需要冷却的材料，优先保证风扇和风道有效覆盖悬垂边。局部风不到位时最容易卷边。' },
      { step: 2, title: '降低悬垂段温度', detail: '温度高时边缘刚出喷嘴就更容易软塌和回卷，适当降5°C常能改善。' },
      { step: 3, title: '降低悬垂速度', detail: '不要整体降速，而是优先对外墙、悬垂和小层区域降速，让喷嘴更稳地铺出边缘。' },
      { step: 4, title: '必要时改朝向或加支撑', detail: '如果角度已经超出机型极限，硬调参数收益很有限，改朝向或轻支撑通常更省时间。' }
    ],
    tips: '悬垂卷边最危险的不是难看，而是它会演变成喷嘴撞件，后面的问题会越来越大。'
  },
  {
    id: 'tree-support-scars',
    video: null,
    category: '切片',
    printerType: 'FDM',
    title: '树状支撑留下疤痕',
    subtitle: '拆完支撑后表面坑坑洼洼',
    emoji: '🌳',
    color: '#66BB6A',
    bgGradient: 'linear-gradient(135deg, #081608 0%, #122710 100%)',
    difficulty: '常见',
    description: '树状支撑虽然省料、好拆，但在接触点附近仍可能留下密集小坑、发白痕和表面起毛，尤其在人物件和曲面件上很明显。',
    causes: ['支撑接触点过密或过大', 'Z距离过小', '拆除方式太暴力', '材料温度和风扇让接触点粘得更紧'],
    solutions: [
      { step: 1, title: '减小支撑接触强度', detail: '适度减小树状支撑尖端直径、降低支撑密度，优先保证“够用”而不是“越多越稳”。' },
      { step: 2, title: '重新评估接触Z距离', detail: '如果总是撕掉表皮，优先把支撑顶层距离放大一点；若已经塌陷，再反向微调回来。' },
      { step: 3, title: '改模型朝向藏疤', detail: '对于手办、外壳、展示件，尽量把需要支撑的面转到背面或不显眼区域，减少后期修补工作。' },
      { step: 4, title: '拆支撑前适当加热或后处理', detail: '某些材料在稍微温热时支撑更好拆；拆完可用热风、刀修和砂纸轻修表面。' }
    ],
    tips: '树状支撑的优势是“整体更友好”，不是“完全无痕”。展示件还是要把支撑痕迹当成设计和摆放的一部分去规划。'
  },
  {
    id: 'bearing-fit-loose',
    video: null,
    category: '切片',
    printerType: 'FDM',
    title: '轴承位过松',
    ...
    emoji: '⚙️',
    color: '#78909C',
    bgGradient: 'linear-gradient(135deg, #0d1114 0%, #182026 100%)',
    difficulty: '进阶',
    description: '模型设计本来想做压入式轴承位，但打印后孔位太松，轴承几乎不用力就能掉出来，无法形成稳定配合。',
    causes: ['孔补偿过头', '外墙实际偏瘦', '材料收缩与设计经验值不匹配', '压入结构壁厚不足'],
    solutions: [
      { step: 1, title: '确认是否整体偏瘦', detail: '先测量外形和孔位，确认是只有轴承孔松，还是整个模型都偏小。若整体偏小，应先做尺寸校准而不是只改单个孔。' },
      { step: 2, title: '回退孔补偿或水平扩展', detail: '如果你已经为了别的结构做过孔补偿，轴承位可能被一起放大过头。对关键孔位最好单独建模或单独补偿。' },
      { step: 3, title: '建立轴承孔经验值', detail: '不同机型和材料对608、625这类常见轴承需要不同干涉量。先打小测试块，找到自己机器合适的压入尺寸。' },
      { step: 4, title: '必要时改为热压或胶固定', detail: '功能件若追求稳定，可在设计上加入止口、卡环或热压套，不必只依赖打印孔的天然压配合。' }
    ],
    tips: '轴承位的配合最好单独建立一套经验尺寸，别和普通“孔能插进去就行”的公差规则混用。'
  },
  {
    id: 'thread-printing-fit',
    video: null,
    category: '切片',
    printerType: 'FDM',
    title: '3D打印螺纹拧不上',
    subtitle: '内外螺纹咬死、发涩或一拧就坏',
    emoji: '🔩',
    color: '#8D6E63',
    bgGradient: 'linear-gradient(135deg, #140d09 0%, #241712 100%)',
    difficulty: '进阶',
    description: '打印出的内外螺纹看起来像样，但实际装配时很难拧入、咬合发涩，甚至一上力就把牙型磨坏。',
    causes: ['螺纹间隙预留不足', '层高过大牙型不清晰', '孔位和外轮廓尺寸误差叠加', '材料本身韧性或脆性不适合细牙'],
    solutions: [
      { step: 1, title: '优先用粗牙和大间隙', detail: '3D打印不适合照搬注塑或金属件的紧配螺纹。建议用粗牙型、较大牙距，并适当增加间隙。' },
      { step: 2, title: '减小层高提升牙型清晰度', detail: '从0.20mm降到0.12或0.16mm后，螺纹侧壁通常会明显更顺，尤其对小尺寸螺纹效果明显。' },
      { step: 3, title: '做螺纹测试样块', detail: '不要第一次就打完整零件，先做短段内外螺纹测试件，验证可旋入性，再决定最终补偿量。' },
      { step: 4, title: '关键连接考虑铜螺母或攻牙', detail: '需要频繁拆装或高强度连接时，热熔铜螺母、嵌件或后攻牙比全打印螺纹更可靠。' }
    ],
    tips: '3D打印螺纹的核心不是“形状做出来”，而是“能顺畅工作还不容易坏”，这通常需要比理论尺寸更大的间隙。'
  },
  {
    id: 'dimensional-x-y-mismatch',
    video: null,
    category: '维护',
    printerType: 'FDM',
    title: 'X/Y 两个方向尺寸不一致',
    subtitle: '同样20mm，一个方向准另一个偏',
    emoji: '📏',
    color: '#5C6BC0',
    bgGradient: 'linear-gradient(135deg, #0b0f1a 0%, #141c2f 100%)',
    difficulty: '需处理',
    description: '校准方块在X方向尺寸正常，Y方向却明显偏差，或者两个方向误差相反，说明问题不只是普通流量偏差。',
    causes: ['某一轴皮带张力异常', '步进参数单轴不准', '滑轨或V轮阻力不一致', '机架不方正'],
    solutions: [
      { step: 1, title: '分别测X/Y方向误差', detail: '连续打印多个标准方块，记录X和Y各自偏差，确认问题是否稳定存在，而不是偶发测量误差。' },
      { step: 2, title: '检查单轴机械状态', detail: '皮带松紧、滑块阻力、V轮压紧和导轨润滑都要分轴检查。某一轴走得更紧或更松，尺寸通常先从那一轴偏掉。' },
      { step: 3, title: '核对步进值与固件设置', detail: '若刚换过皮带轮、步进细分或固件，先确认X/Y steps/mm没有配置错误。' },
      { step: 4, title: '检查机架垂直和平行', detail: '方架不方、龙门歪斜会让两个方向表现不一致，尤其在大尺寸件上更明显。' }
    ],
    tips: '如果X和Y误差不同，就别只调流量。流量更像“整体胖瘦”，而单轴误差通常是机械或步进配置问题。'
  },
  {
    id: 'extruder-gear-grinding',
    video: null,
    category: '维护',
    printerType: 'FDM',
    title: '挤出齿轮磨料',
    subtitle: '耗材被咬出深槽，越打越送不动',
    emoji: '🪚',
    color: '#FFA726',
    bgGradient: 'linear-gradient(135deg, #1a1004 0%, #2d1d08 100%)',
    difficulty: '需处理',
    description: '挤出机齿轮持续在同一位置磨耗材，表面被咬出很深的粉末和沟槽，最后完全失去推进能力。',
    causes: ['喷嘴或料路阻力过大', '压紧力过强', '回抽频繁导致局部磨损', '齿轮槽内积粉严重'],
    solutions: [
      { step: 1, title: '先查下游阻力', detail: '齿轮磨料往往不是齿轮本身的问题，而是后面推不动。先排查喷嘴半堵、热端过冷、PTFE老化和线盘阻力。' },
      { step: 2, title: '清理齿轮槽和碎屑', detail: '停机后用刷子或针清掉齿轮上的塑料粉末，否则咬合会越来越浅，打滑更严重。' },
      { step: 3, title: '回退压紧力一点点', detail: '如果耗材被咬得太深，适当放松挤出机弹簧，让齿轮既能抓住料，又不至于把它磨扁。' },
      { step: 4, title: '减少无意义高频回抽', detail: '很多小碎模型会让挤出机在同一小段耗材上疯狂来回拉扯，软料和脆料更容易被磨坏。' }
    ],
    tips: '看到“磨出一圈塑料粉”时，别只怪齿轮脏。真正的问题通常是它为什么需要那么费劲地推料。'
  },
  {
    id: 'belt-ringing-highspeed',
    video: null,
    category: '进阶',
    printerType: 'FDM',
    title: '高速打印振纹加重',
    subtitle: '提速后立面出现一圈圈回声纹',
    emoji: '🏎️',
    color: '#29B6F6',
    bgGradient: 'linear-gradient(135deg, #06121a 0%, #0b2130 100%)',
    difficulty: '进阶',
    description: '同一个模型在普通速度下还能接受，一旦提高速度和加速度，立面拐角后就出现更明显的“回声纹”或振纹。',
    causes: ['皮带张力不理想', '机架共振明显', '输入整形未校准', '高加速度超出结构承受范围'],
    solutions: [
      { step: 1, title: '重新确认皮带状态', detail: '皮带太松和太紧都可能放大高速振动。先恢复到机型推荐张力，再看振纹有没有明显变化。' },
      { step: 2, title: '降低加速度优先于降最高速度', detail: '很多振纹不是由巡航速度造成，而是由急加减速触发。先把加速度和急动速度降一点更有效。' },
      { step: 3, title: '有条件就做输入整形', detail: 'Klipper或支持共振补偿的机型可通过加速度传感器校准输入整形，大幅压低高速振纹。' },
      { step: 4, title: '检查结构件与平台稳定性', detail: '桌面晃、机脚太软、龙门固定松动都会让提速后的共振成倍放大。' }
    ],
    tips: '“高速打印不好看”不一定是机器不行，很多时候只是你还没找到这台机子的稳定速度区间和共振补偿参数。'
  },
  {
    id: 'resin-blooming',
    video: null,
    ...
    printerType: 'SLA',
    title: '光固化局部泛白结晶',
    subtitle: '表面出现斑块、发雾或白色沉积',
    emoji: '🧊',
    color: '#90CAF9',
    bgGradient: 'linear-gradient(135deg, #0a1117 0%, #12202b 100%)',
    difficulty: '常见',
    description: '模型表面不是整体发白，而是局部出现片状雾面、斑块或像结晶一样的白色附着，成品观感很差。',
    causes: ['树脂与酒精残留反应', '清洗液污染严重', '固化前表面有残液堆积', '后处理温度和时间过猛'],
    solutions: [
      { step: 1, title: '使用两段式清洗', detail: '先粗洗去掉大部分树脂，再用第二桶干净酒精精洗，减少脏液在表面留下沉积。' },
      { step: 2, title: '特别注意沟槽和凹面残液', detail: '局部泛白常出现在酒精和树脂容易积聚的地方。清洗后要把这些位置吹干，别让液滴停在表面。' },
      { step: 3, title: '缩短单次固化时间', detail: '分段固化比一次猛照更不容易出现局部泛白和热积累，尤其对透明树脂和浅色树脂更明显。' },
      { step: 4, title: '污染严重时整桶更换清洗液', detail: '如果一整批模型都开始发白，不要再靠延长晾干时间硬扛，优先换清洗液。' }
    ],
    tips: '这种局部泛白很像曝光问题，但很多时候根源其实在后处理流程，尤其是“清洗液太脏还舍不得换”。'
  },
  {
    id: 'resin-cupping-holes',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '中空件排液孔位置不对',
    subtitle: '明明开了孔，还是吸盘或洗不干净',
    emoji: '🕳️',
    color: '#7CB342',
    bgGradient: 'linear-gradient(135deg, #0b1508 0%, #15260f 100%)',
    difficulty: '进阶',
    description: '模型虽然已经设计了排液孔，但打印时仍然出现吸盘效应、内腔积液或后期炸裂，说明问题不在“有没有孔”，而在孔的位置和流路。',
    causes: ['孔位开在高处排不出液', '只有一个孔无法形成进出流', '孔径太小', '模型姿态让内腔形成死角'],
    solutions: [
      { step: 1, title: '至少建立进出双孔', detail: '一个孔常常只能排一部分液，至少要形成液体能进能出的通路，清洗和排液才真正有效。' },
      { step: 2, title: '让孔位处在实际最低点', detail: '孔不是随便开在“底部看起来差不多”的位置，而是要对应打印姿态下内腔液体真正汇集的最低点。' },
      { step: 3, title: '避免内腔死角', detail: '如果模型内部有多腔室或转折，单个孔未必能覆盖全部区域，必要时为每个腔室单独设计排液路径。' },
      { step: 4, title: '用切片预览检查姿态下液体路径', detail: '结合切片方向思考“树脂会往哪流、空气从哪进”，比单纯凭建模时的正视图更可靠。' }
    ],
    tips: '中空件最常见的误区就是“我已经开孔了为什么还出事”。很多时候不是没开，而是开的地方根本不工作。'
  },
  {
    id: 'resin-layer-lines',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '光固化层纹明显',
    subtitle: '斜面和曲面出现一阶一阶台阶纹',
    emoji: '📚',
    color: '#7986CB',
    bgGradient: 'linear-gradient(135deg, #0c1018 0%, #151c2c 100%)',
    difficulty: '常见',
    description: '光固化件虽然细节清楚，但斜面、球面和大曲面上能明显看到一层一层的台阶感，影响外观。',
    causes: ['层高过大', '模型摆放角度不佳', '抗锯齿设置不足', '后处理放大了表面反光'],
    solutions: [
      { step: 1, title: '降低层高', detail: '从0.05mm降到0.03mm或0.02mm后，层纹会明显变细，但打印时间会显著增加。' },
      { step: 2, title: '调整模型摆放角度', detail: '斜放通常比正放更能分散台阶纹，让层纹不集中出现在最显眼的大平面或曲面上。' },
      { step: 3, title: '启用抗锯齿与灰阶补偿', detail: '部分机器和切片器支持边缘灰阶或抗锯齿，对视觉层纹改善明显，尤其是浅色模型。' },
      { step: 4, title: '后处理时做轻打磨或喷底漆', detail: '展示件可通过底漆填纹、细砂纸和补土进一步降低层纹存在感。' }
    ],
    tips: '光固化并不是天然“完全没层纹”，只是层纹更细。摆放角度和层高仍然会直接决定表面观感。'
  },
  {
    id: 'resin-support-pitting',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '光固化支撑点凹坑',
    subtitle: '拆支撑后留下一个个小坑',
    emoji: '🩹',
    color: '#BA68C8',
    bgGradient: 'linear-gradient(135deg, #12081a 0%, #20102c 100%)',
    difficulty: '常见',
    description: '模型拆支撑后不是简单留白点，而是形成肉眼明显的小坑、小坑边缘撕裂或局部缺肉，修复成本很高。',
    causes: ['支撑接触点过深过粗', '支撑拆除时机不对', '模型太脆或二固后才拆', '受力方向不合理'],
    solutions: [
      { step: 1, title: '优先减小接触点和插入深度', detail: '支撑点不是越稳越好，能承住就行。关键展示面应尽量使用更细、更浅的接触点。' },
      { step: 2, title: '尽量在半固化或刚清洗后拆支撑', detail: '很多树脂在完全二固前更有韧性，此时拆支撑更容易整点脱离，减少崩坑。' },
      { step: 3, title: '调整受力方向再拆', detail: '不要直接朝外硬掰，可顺着支撑生长方向或先剪短再逐个拆，降低带走表皮的风险。' },
      { step: 4, title: '把支撑点藏到背面或后加工面', detail: '人物脸、光滑外壳、镜面区域尽量避免放主支撑点，把伤害留给后续能修的面。' }
    ],
    tips: '树脂支撑痕很难完全消失，所以最好的思路不是“后面修”，而是从一开始就把它安排在不重要的位置。'
  },
  {
    id: 'bridge-sagging',
    video: null,
    ...
    printerType: 'FDM',
    title: '桥接下垂',
    subtitle: '跨空位置像挂帘一样往下塌',
    emoji: '🌉',
    color: '#4FC3F7',
    bgGradient: 'linear-gradient(135deg, #07141a 0%, #0c2431 100%)',
    difficulty: '常见',
    description: '两个支点之间跨空打印时，挤出的线条无法被很好拉直，底面像拉丝布帘一样下垂，严重时直接塌陷。',
    causes: ['桥接风冷不足', '桥接速度太慢', '喷嘴温度偏高', '跨距超出当前工艺能力'],
    solutions: [
      { step: 1, title: '单独优化桥接参数', detail: '在切片器中为桥接设置更高风扇、更低流量和更快桥接速度，不要沿用普通外墙参数。' },
      { step: 2, title: '小步降低喷嘴温度', detail: '桥接需要挤出后尽快定型，温度过高会让线材像面条一样软塌。每次降低 5°C 观察变化。' },
      { step: 3, title: '让桥接线先拉紧再落下', detail: '部分机型在桥接时提高移动速度反而更稳定，因为线条会被“拉”在两端，而不是慢慢坠下。' },
      { step: 4, title: '调整模型姿态或拆分打印', detail: '如果桥接距离过长，单靠参数很难彻底解决。可改变摆放方向，或拆件后再组装。' }
    ],
    tips: '桥接不是越慢越好。很多新手直觉会降速，其实桥接更常见的问题是“线还没拉起来就先塌了”。'
  },
  {
    id: 'z-banding',
    video: null,
    ...
    printerType: 'FDM',
    title: 'Z 纹 / 立面周期波纹',
    subtitle: '外墙出现规则横纹或明暗带',
    emoji: '📏',
    color: '#64B5F6',
    bgGradient: 'linear-gradient(135deg, #08111a 0%, #102030 100%)',
    difficulty: '进阶',
    description: '模型外墙不是随机粗糙，而是随着高度出现有规律的横向波纹、明暗带或周期性尺寸变化，立面观感很差。',
    causes: ['Z 轴丝杆偏摆或不顺直', '联轴器受力不当', '导轨阻力不均', '层高与机械节距叠加放大误差'],
    solutions: [
      { step: 1, title: '先区分是机械周期纹还是切片接缝', detail: '如果条纹沿整圈持续出现且有固定节距，更像 Z 轴机械问题；若只在一条竖线附近明显，则先看接缝。' },
      { step: 2, title: '检查丝杆、联轴器和固定方式', detail: '观察丝杆是否摆动、联轴器是否顶死电机轴或丝杆，避免把偏摆强行传给 Z 轴运动。' },
      { step: 3, title: '清洁并润滑 Z 轴导向件', detail: '干涩、阻力不均会让每层抬升不是线性的，表现为外墙周期纹加重。' },
      { step: 4, title: '换一个常用层高做对比', detail: '某些层高会放大机械误差。尝试从 0.2mm 改到 0.16mm 或 0.24mm，帮助判断是否为叠加共振。' }
    ],
    tips: '看到横纹不要第一时间只调流量。真正周期性的 Z 纹，大多数根因还是在机械传动链。'
  },
  {
    id: 'corner-bulge',
    video: null,
    ...
    printerType: 'FDM',
    title: '转角鼓包',
    subtitle: '外墙拐角变圆、堆料明显',
    emoji: '📦',
    color: '#FFB74D',
    bgGradient: 'linear-gradient(135deg, #1a1207 0%, #2c1f0d 100%)',
    difficulty: '进阶',
    description: '方块、壳体等模型的外轮廓在转角处明显比直线段更鼓，导致拐角发胖、边缘不利落，尺寸也会跟着偏大。',
    causes: ['压力提前量未校准', '外墙速度与加速度过高', '喷嘴内压释放不及时', '过挤出放大转角堆料'],
    solutions: [
      { step: 1, title: '先做压力提前量测试', detail: '如果固件支持 Pressure Advance / Linear Advance，优先通过测试塔找到更合适的值。' },
      { step: 2, title: '降低外墙加速度', detail: '拐角鼓包很多时候不是巡航速度的问题，而是减速入弯时内压还没来得及卸掉。' },
      { step: 3, title: '复查流量和外墙温度', detail: '如果整体就偏多料，转角会被进一步放大。先把单壁和外墙挤出调到比较干净。' },
      { step: 4, title: '必要时单独降外墙速度', detail: '结构件追求尺寸时，可单独把外墙速度压低，让转角线更可控。' }
    ],
    tips: '“转角鼓包”和“接缝鼓包”看起来像，但前者更偏几何和内压控制，后者更偏接缝策略和停顿。'
  },
  {
    id: 'resin-overcured-brittle',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '树脂件过固发脆',
    subtitle: '打印出来能成型，但很容易崩裂',
    emoji: '🧱',
    color: '#CE93D8',
    bgGradient: 'linear-gradient(135deg, #120a17 0%, #23122c 100%)',
    difficulty: '进阶',
    description: '树脂件外观看起来完整，但韧性很差，拆支撑、轻微弯折或装配时就容易崩边、开裂甚至整件断掉。',
    causes: ['曝光参数偏高', '二次固化时间过长', '树脂本身偏脆', '局部壁厚过薄又承担受力'],
    solutions: [
      { step: 1, title: '重新做曝光测试', detail: '如果细节发糊、边缘偏钝又很脆，通常意味着曝光窗口偏高了，不要只靠经验值硬打。' },
      { step: 2, title: '缩短二次固化并分段进行', detail: '后固化不是越久越好。很多树脂在过度固化后会明显发脆，尤其是薄壁件。' },
      { step: 3, title: '按用途选择更耐冲击树脂', detail: '展示树脂和工程树脂的韧性差异很大，承担装配或卡扣的零件不要只图表面好看。' },
      { step: 4, title: '优化受力结构和壁厚', detail: '过薄的连接处即使曝光正确也可能脆断。适当增厚根部、倒圆角能明显提升耐用性。' }
    ],
    tips: '树脂件“能打出来”不等于“参数就对了”。很多脆裂问题其实是曝光和后处理一起堆出来的。'
  },
  {
    id: 'resin-cloudy-surface',
    video: null,
    category: '材料',
    printerType: 'SLA',
    title: '树脂件表面发雾',
    subtitle: '透明件不透亮，外表像蒙了一层灰',
    emoji: '🌫️',
    color: '#B0BEC5',
    bgGradient: 'linear-gradient(135deg, #0d1317 0%, #172127 100%)',
    difficulty: '常见',
    description: '模型表面没有明显结构错误，但整体发雾、透光性差或像有一层白蒙蒙的膜，尤其在透明树脂上更明显。',
    causes: ['清洗残液未吹干', '清洗液污染严重', '后固化过猛导致表面失透', '表面微纹和支撑痕反光放大'],
    solutions: [
      { step: 1, title: '清洗后彻底吹干再固化', detail: '残留酒精和树脂混合物会直接在表面形成雾感，尤其是凹槽和孔内最容易藏液。' },
      { step: 2, title: '更换更干净的清洗液', detail: '透明件对脏液特别敏感，清洗液稍微发黄或混浊，就可能明显降低透亮度。' },
      { step: 3, title: '降低固化强度并缩短时长', detail: '透明树脂过固后表面容易失透，可采用更短、更均匀的分段固化。' },
      { step: 4, title: '需要展示效果时做精修抛光', detail: '从细砂纸到抛光膏逐步处理，可显著恢复透明件透光感。' }
    ],
    tips: '透明树脂想做好看，打印只是第一步，清洗和后处理的影响经常比曝光参数还大。'
  },
]

const STRUCTURED_META_BY_ID = {
  warping: {
    stages: ['first-layer'],
    symptomTags: ['corners-lifting', 'edge-curl', 'base-detach'],
    materials: ['pla', 'petg', 'abs-asa', 'any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先稳首层，再加 Brim，再控环境温差',
    firstAction: '先检查首层压实程度，并确认热床已经彻底清洁。',
    relatedIds: ['first-layer-not-sticking', 'elephant-foot'],
    commonMisdiagnosis: ['把翘边误当成单纯的热床脏', '只加胶不调首层和环境温差'],
    searchAliases: ['卷边', '四角翘起', '边角抬起'],
  },
  'first-layer-not-sticking': {
    stages: ['before', 'first-layer'],
    symptomTags: ['base-detach', 'first-layer-gaps', 'line-dragging'],
    materials: ['pla', 'petg', 'abs-asa', 'tpu', 'any'],
    estimatedTime: '约 5 分钟',
    checkOrder: '先清洁热床，再降速，再调温度',
    firstAction: '先用 IPA 清洁热床，再把首层速度降到正常值的一半以下。',
    relatedIds: ['warping', 'warped-bed'],
    commonMisdiagnosis: ['把不粘床误判为喷嘴堵塞', '只调温度不看 Z 偏移'],
    searchAliases: ['不粘床', '粘不住', '首层被带走'],
  },
  'elephant-foot': {
    stages: ['first-layer'],
    symptomTags: ['bottom-bulge', 'fit-too-tight'],
    materials: ['pla', 'petg', 'abs-asa', 'any'],
    estimatedTime: '约 6 分钟',
    checkOrder: '先抬 Z，再降床温，再做补偿',
    firstAction: '先轻微抬高 Z 偏移，观察底层是否不再被压得发亮发扁。',
    relatedIds: ['first-layer-lines', 'tolerance-calibration'],
    commonMisdiagnosis: ['把象脚误当成整体尺寸缩放问题'],
    searchAliases: ['底部变胖', '底层外扩'],
  },
  'first-layer-lines': {
    stages: ['before', 'first-layer'],
    symptomTags: ['first-layer-gaps', 'line-width-uneven', 'patchy-first-layer'],
    materials: ['any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先打首层测试，再调 Z 偏移，再复查网格',
    firstAction: '先打印一张首层测试片，一边打印一边微调 Baby Step。',
    relatedIds: ['warped-bed', 'first-layer-not-sticking'],
    commonMisdiagnosis: ['把局部线宽不均都归因于耗材质量'],
    searchAliases: ['首层不均匀', '第一层宽窄不一'],
  },
  'warped-bed': {
    stages: ['first-layer'],
    symptomTags: ['patchy-first-layer', 'local-high-low-bed'],
    materials: ['any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先查床网格，再查局部高低差',
    firstAction: '先重新做床网格调平，确认异常位置是否固定重现。',
    relatedIds: ['first-layer-lines', 'first-layer-not-sticking'],
    commonMisdiagnosis: ['把局部不平误认成单纯附着问题'],
    searchAliases: ['热床不平', '局部太挤', '局部悬空'],
  },
  stringing: {
    stages: ['surface'],
    symptomTags: ['hair-stringing', 'travel-webs'],
    materials: ['pla', 'petg', 'tpu', 'abs-asa', 'any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先降温，再调回抽，再提 travel',
    firstAction: '先把喷嘴温度降低 5°C，再用同一个模型复打对比。',
    relatedIds: ['wet-filament', 'petg-stringing'],
    commonMisdiagnosis: ['把受潮拉丝误当成单纯回抽不足'],
    searchAliases: ['拉丝', '蜘蛛网', '细线'],
  },
  'wet-filament': {
    stages: ['surface'],
    symptomTags: ['hair-stringing', 'surface-bubbles', 'popping-marks', 'rough-surface'],
    materials: ['pla', 'petg', 'abs-asa', 'tpu', 'any'],
    estimatedTime: '约 30-60 分钟',
    checkOrder: '先烘干，再复打对比件',
    firstAction: '先烘干耗材，再用原切片打印一件小测试件复核差异。',
    relatedIds: ['stringing', 'top-surface-rough'],
    commonMisdiagnosis: ['把噼啪和粗糙都误认成温度不对'],
    searchAliases: ['受潮', '爆裂声', '气泡'],
  },
  'petg-stringing': {
    stages: ['surface'],
    symptomTags: ['hair-stringing', 'travel-webs'],
    materials: ['petg'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先压温度，再控速度与回抽',
    firstAction: '先把 PETG 温度往下压 5°C，再观察空移拉丝的改善程度。',
    relatedIds: ['stringing', 'wet-filament'],
    commonMisdiagnosis: ['套用 PLA 回抽参数直接打印 PETG'],
    searchAliases: ['PETG拉丝'],
  },
  'under-extrusion': {
    stages: ['before', 'mid-print', 'surface'],
    symptomTags: ['gaps-in-walls', 'thin-lines', 'top-holes', 'rough-surface'],
    materials: ['any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先查喷嘴，再查送丝，再查温度',
    firstAction: '先手动挤出一段耗材，确认出丝是否持续、顺畅、线径稳定。',
    relatedIds: ['clogged-nozzle', 'extruder-clicking'],
    commonMisdiagnosis: ['把欠挤出误判为层高太大'],
    searchAliases: ['缺料', '出丝少', '线条稀疏'],
  },
  'clogged-nozzle': {
    stages: ['before', 'mid-print'],
    symptomTags: ['thin-lines', 'gaps-in-walls', 'partial-clog'],
    materials: ['any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先冷拔，再清喷嘴，再换喷嘴',
    firstAction: '先做一次冷拔，确认是不是部分堵塞。',
    relatedIds: ['under-extrusion', 'no-extrusion'],
    commonMisdiagnosis: ['把堵头误判成纯粹温度低'],
    searchAliases: ['堵嘴', '堵头', '半堵'],
  },
  'no-extrusion': {
    stages: ['before', 'mid-print'],
    symptomTags: ['no-filament', 'line-dragging'],
    materials: ['any'],
    estimatedTime: '约 5 分钟',
    checkOrder: '先查供料，再查喷嘴，再查 Z 高度',
    firstAction: '先确认线盘、进料口和喷嘴三个位置是否至少有一个明显卡点。',
    relatedIds: ['clogged-nozzle', 'filament-tangle'],
    commonMisdiagnosis: ['只看喷嘴，不看线盘或进料通道'],
    searchAliases: ['不出丝', '空打', '没料'],
  },
  'filament-tangle': {
    stages: ['before', 'mid-print'],
    symptomTags: ['no-filament', 'filament-pulled-tight'],
    materials: ['any'],
    estimatedTime: '约 3 分钟',
    checkOrder: '先看线盘，再理料路，再重试挤出',
    firstAction: '先把线盘手动顺出几圈，确认是否存在死结和跨圈压线。',
    relatedIds: ['no-extrusion'],
    commonMisdiagnosis: ['把线盘打结误判成喷嘴堵塞'],
    searchAliases: ['线盘打结', '卡料'],
  },
  'layer-shift': {
    stages: ['mid-print'],
    symptomTags: ['layer-shift', 'offset-wall', 'crash-mark'],
    materials: ['any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先查碰撞，再查皮带，再降加速度',
    firstAction: '先看错位前的高度和模型位置，确认是否发生过喷嘴撞件。',
    relatedIds: ['motor-stall', 'spaghetti'],
    commonMisdiagnosis: ['把层移直接当成切片文件损坏'],
    searchAliases: ['错位', '偏层', '移位'],
  },
  spaghetti: {
    stages: ['mid-print'],
    symptomTags: ['spaghetti', 'collapse-failure'],
    materials: ['any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先停机清理，再回看失效起点',
    firstAction: '先停机清喷嘴和热床，再回想它是从哪一层开始失控的。',
    relatedIds: ['warping', 'support-optimization'],
    commonMisdiagnosis: ['把意大利面当成单一问题，其实它只是失败结果'],
    searchAliases: ['面条', '打面', '坍塌'],
  },
  'extruder-clicking': {
    stages: ['mid-print'],
    symptomTags: ['extruder-clicking', 'thin-lines'],
    materials: ['any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先看堵头，再查温度，再查回抽',
    firstAction: '先检查喷嘴是否存在局部堵塞，再看当前温度是否压得太低。',
    relatedIds: ['clogged-nozzle', 'heat-creep'],
    commonMisdiagnosis: ['把咔咔响只当成挤出机弹簧太紧'],
    searchAliases: ['咔咔响', '打滑'],
  },
  'heat-creep': {
    stages: ['mid-print'],
    symptomTags: ['extruder-clicking', 'partial-clog', 'late-failure'],
    materials: ['pla', 'tpu'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先查热端散热，再做长时验证',
    firstAction: '先检查热端风扇和散热片是否积灰、转速异常或方向不对。',
    relatedIds: ['clogged-nozzle', 'extruder-clicking'],
    commonMisdiagnosis: ['把热蠕变误判成单纯回抽过大'],
    searchAliases: ['热蠕变', '打久了堵头'],
  },
  'blobs-zits': {
    stages: ['surface'],
    symptomTags: ['surface-blobs', 'seam-bump'],
    materials: ['any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先看接缝，再查停顿，再调回抽',
    firstAction: '先确认鼓包是不是总出现在同一条竖线上。',
    relatedIds: ['seam-visible', 'corner-bulge'],
    commonMisdiagnosis: ['把接缝鼓包误当成全局过挤出'],
    searchAliases: ['疙瘩', '痘点', '鼓包'],
  },
  'seam-visible': {
    stages: ['surface'],
    symptomTags: ['seam-bump', 'single-vertical-line'],
    materials: ['any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先改接缝位置，再复打立面件',
    firstAction: '先把接缝策略改成后侧或随机，观察竖线是否转移。',
    relatedIds: ['blobs-zits', 'corner-bulge'],
    commonMisdiagnosis: ['把接缝线误判成 Z 纹'],
    searchAliases: ['接缝明显', '竖线明显'],
  },
  'top-surface-rough': {
    stages: ['surface'],
    symptomTags: ['top-holes', 'rough-surface'],
    materials: ['any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先加顶层，再查流量与顶面速度',
    firstAction: '先把顶层层数加厚，再看顶面是否能完整盖住填充。',
    relatedIds: ['under-extrusion', 'wet-filament'],
    commonMisdiagnosis: ['把顶面发毛误判为单纯风扇太大'],
    searchAliases: ['顶面粗糙', '封顶不好'],
  },
  'tolerance-calibration': {
    stages: ['surface'],
    symptomTags: ['fit-too-tight', 'dimension-off', 'holes-too-small'],
    materials: ['any'],
    estimatedTime: '约 15 分钟',
    checkOrder: '先打尺寸块，再调 Flow，再调 XY',
    firstAction: '先打一块 20mm 校准块并实际量取 X、Y、Z 尺寸。',
    relatedIds: ['shrinkage-tolerance', 'estep-calibration'],
    commonMisdiagnosis: ['把孔小和外轮廓偏大都归因于模型设计'],
    searchAliases: ['尺寸不准', '配合太紧', '孔位偏小'],
  },
  'shrinkage-tolerance': {
    stages: ['surface'],
    symptomTags: ['dimension-off', 'holes-too-small'],
    materials: ['pla', 'petg', 'abs-asa', 'tpu'],
    estimatedTime: '约 15 分钟',
    checkOrder: '先测材料收缩，再做孔径补偿',
    firstAction: '先用当前材料打尺寸件，不要直接沿用上一个材料的补偿值。',
    relatedIds: ['tolerance-calibration'],
    commonMisdiagnosis: ['把材料收缩误判为机器步进不准'],
    searchAliases: ['收缩补偿'],
  },
  'estep-calibration': {
    stages: ['surface'],
    symptomTags: ['dimension-off', 'thin-lines'],
    materials: ['any'],
    estimatedTime: '约 15 分钟',
    checkOrder: '先测 100mm 挤出，再修正 E 步',
    firstAction: '先标记 100mm 耗材并实测挤出长度。',
    relatedIds: ['tolerance-calibration', 'under-extrusion'],
    commonMisdiagnosis: ['跳过 E 步直接疯狂调 Flow'],
    searchAliases: ['E步校准', '挤出步进'],
  },
  'missing-thin-details': {
    stages: ['surface'],
    symptomTags: ['detail-loss', 'soft-edges'],
    materials: ['resin'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先查摆放，再查特征尺寸与层高',
    firstAction: '先看丢失的是不是已经接近当前工艺最小特征极限。',
    relatedIds: ['resin-exposure', 'resin-layer-lines'],
    commonMisdiagnosis: ['把细节丢失都归因于机器分辨率不够'],
    searchAliases: ['细节丢失', '文字发糊'],
  },
  'resin-exposure': {
    stages: ['before', 'first-layer', 'surface'],
    symptomTags: ['soft-edges', 'detail-loss', 'fep-sticking'],
    materials: ['resin'],
    estimatedTime: '约 15 分钟',
    checkOrder: '先打曝光测试，再微调底层与普通层',
    firstAction: '先做一块曝光测试片，不要直接改大模型参数。',
    relatedIds: ['resin-fep-failure', 'resin-overcured-brittle'],
    commonMisdiagnosis: ['把曝光不足和支撑不足混在一起看'],
    searchAliases: ['曝光不够', '曝光过度'],
  },
  'resin-fep-failure': {
    stages: ['before', 'first-layer'],
    symptomTags: ['fep-sticking', 'base-detach'],
    materials: ['resin'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先查 FEP 状态，再查离型阻力',
    firstAction: '先看 FEP 是否发白、划伤、过松或过紧。',
    relatedIds: ['resin-exposure', 'resin-suction-cup'],
    commonMisdiagnosis: ['把粘膜问题都归结为曝光不足'],
    searchAliases: ['粘FEP', '粘膜'],
  },
  'resin-warping': {
    stages: ['first-layer', 'mid-print'],
    symptomTags: ['edge-curl', 'support-failure', 'collapse-failure'],
    materials: ['resin'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先查支撑，再查摆放角度',
    firstAction: '先回看支撑是否覆盖了主要受力边和长悬臂区域。',
    relatedIds: ['resin-suction-cup', 'resin-fep-failure'],
    commonMisdiagnosis: ['把树脂翘曲都看成曝光不对'],
    searchAliases: ['树脂翘曲', '树脂变形'],
  },
  'resin-suction-cup': {
    stages: ['mid-print'],
    symptomTags: ['collapse-failure', 'fep-sticking', 'hollow-trap'],
    materials: ['resin'],
    estimatedTime: '约 15 分钟',
    checkOrder: '先查吸盘腔，再补泄压孔与角度',
    firstAction: '先判断当前姿态下是否形成了封闭吸盘腔。',
    relatedIds: ['resin-cupping-holes', 'resin-warping'],
    commonMisdiagnosis: ['只加支撑，不去消除吸盘结构'],
    searchAliases: ['吸盘效应', '树脂脱落'],
  },
  'resin-wash-cure': {
    stages: ['surface'],
    symptomTags: ['soft-edges', 'white-bloom', 'surface-cloudy'],
    materials: ['resin'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先减清洗固化，再看后处理节奏',
    firstAction: '先缩短清洗和固化时长，再对比表面和韧性的变化。',
    relatedIds: ['resin-blooming', 'resin-cloudy-surface'],
    commonMisdiagnosis: ['把后处理问题误判成打印过程曝光问题'],
    searchAliases: ['后处理过度'],
  },
  'resin-layer-lines': {
    stages: ['surface'],
    symptomTags: ['visible-layers', 'stair-steps'],
    materials: ['resin'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先降层高，再改摆放，再看抗锯齿',
    firstAction: '先确认这是不是由摆放角度导致的视觉台阶集中。',
    relatedIds: ['missing-thin-details'],
    commonMisdiagnosis: ['把层纹完全归因于机器精度'],
    searchAliases: ['层纹明显', '台阶纹'],
  },
  'resin-support-pitting': {
    stages: ['surface'],
    symptomTags: ['support-pits', 'surface-scars'],
    materials: ['resin'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先看接触点，再改拆支撑时机',
    firstAction: '先确认是不是在完全二固后才拆支撑，导致表皮更脆。',
    relatedIds: ['resin-wash-cure'],
    commonMisdiagnosis: ['把支撑坑误认成树脂本身太差'],
    searchAliases: ['支撑坑', '拆支撑小坑'],
  },
  'resin-blooming': {
    stages: ['surface'],
    symptomTags: ['white-bloom', 'surface-cloudy'],
    materials: ['resin'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先换清洗液，再看吹干和固化节奏',
    firstAction: '先检查是不是脏酒精和残液共同导致了局部泛白。',
    relatedIds: ['resin-cloudy-surface', 'resin-wash-cure'],
    commonMisdiagnosis: ['把泛白当成曝光过高'],
    searchAliases: ['泛白', '发雾', '结晶'],
  },
  'resin-cupping-holes': {
    stages: ['mid-print', 'surface'],
    symptomTags: ['hollow-trap', 'collapse-failure'],
    materials: ['resin'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先查排液孔位置，再查姿态流路',
    firstAction: '先判断当前打印姿态下，排液孔是不是处在真正的最低点。',
    relatedIds: ['resin-suction-cup'],
    commonMisdiagnosis: ['以为只要开孔就一定能排液'],
    searchAliases: ['排液孔', '中空件吸盘'],
  },
  'nozzle-scraping-print': {
    stages: ['mid-print'],
    symptomTags: ['crash-mark', 'surface-scars'],
    materials: ['any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先查翘边或鼓包，再查 Z 跳抬与轨迹',
    firstAction: '先看喷嘴是固定刮某一处，还是整圈都在摩擦模型表面。',
    relatedIds: ['layer-shift', 'corner-bulge'],
    commonMisdiagnosis: ['把刮擦只归因于 Z 偏移没调好'],
    searchAliases: ['喷嘴刮模型', '打到模型'],
  },
  'wall-delamination-corners': {
    stages: ['surface'],
    symptomTags: ['corner-crack', 'gaps-in-walls'],
    materials: ['abs-asa', 'pla', 'petg', 'any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先看温度和风冷，再看壳壁路径',
    firstAction: '先判断裂纹是不是集中在尖角和受力转折位置。',
    relatedIds: ['layer-separation', 'warping'],
    commonMisdiagnosis: ['把拐角开裂误当成单纯模型太薄'],
    searchAliases: ['墙角开裂', '拐角分层'],
  },
  'tree-support-scars': {
    stages: ['surface'],
    symptomTags: ['support-scars', 'surface-scars'],
    materials: ['pla', 'petg', 'abs-asa', 'tpu', 'any'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先调支撑接触，再改摆放和支撑面位置',
    firstAction: '先看伤痕是不是都集中在支撑接触最密集的面上。',
    relatedIds: ['support-removal', 'overhang-curling'],
    commonMisdiagnosis: ['把支撑疤痕误判成冷却不良'],
    searchAliases: ['树状支撑痕', '支撑疤'],
  },
  'overhang-curling': {
    stages: ['surface', 'mid-print'],
    symptomTags: ['overhang-curl', 'surface-scars'],
    materials: ['pla', 'petg', 'abs-asa', 'any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先看冷却，再看层厚和速度',
    firstAction: '先确认问题是否发生在下垂悬垂区域的边缘卷起处。',
    relatedIds: ['bridge-sagging', 'nozzle-scraping-print'],
    commonMisdiagnosis: ['把悬垂卷边误看成整体温度过低'],
    searchAliases: ['悬垂卷边', '下垂边卷起'],
  },
  'bridge-sagging': {
    stages: ['surface'],
    symptomTags: ['bridge-sag', 'overhang-curl'],
    materials: ['pla', 'petg', 'abs-asa', 'any'],
    estimatedTime: '约 10 分钟',
    checkOrder: '先提桥接风冷，再查桥接速度和温度',
    firstAction: '先单独检查切片器里是否启用了桥接参数。',
    relatedIds: ['overhang-curling'],
    commonMisdiagnosis: ['桥接塌陷时盲目降速'],
    searchAliases: ['拉桥失败', '桥接下垂'],
  },
  'z-banding': {
    stages: ['surface'],
    symptomTags: ['visible-layers', 'periodic-bands'],
    materials: ['any'],
    estimatedTime: '约 15 分钟',
    checkOrder: '先排接缝，再查丝杆和导向',
    firstAction: '先确认条纹是不是整圈连续且有固定节距。',
    relatedIds: ['seam-visible'],
    commonMisdiagnosis: ['把周期性 Z 纹误看成接缝线'],
    searchAliases: ['Z纹', '周期横纹', '立面波纹'],
  },
  'corner-bulge': {
    stages: ['surface'],
    symptomTags: ['corner-bulge', 'surface-blobs', 'dimension-off'],
    materials: ['any'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先校压力提前量，再降外墙加速度',
    firstAction: '先观察鼓包是否只在直角、急转角位置明显。',
    relatedIds: ['pressure-advance', 'blobs-zits'],
    commonMisdiagnosis: ['把转角鼓包全部误当成接缝问题'],
    searchAliases: ['拐角鼓包', '转角发胖'],
  },
  'resin-overcured-brittle': {
    stages: ['surface'],
    symptomTags: ['brittle-crack', 'soft-edges'],
    materials: ['resin'],
    estimatedTime: '约 12 分钟',
    checkOrder: '先复查曝光，再收缩后固化时长',
    firstAction: '先减少二次固化时间，观察脆性是否立刻改善。',
    relatedIds: ['resin-exposure', 'resin-wash-cure'],
    commonMisdiagnosis: ['把发脆全怪树脂品牌'],
    searchAliases: ['过固发脆', '一掰就裂'],
  },
  'resin-cloudy-surface': {
    stages: ['surface'],
    symptomTags: ['surface-cloudy', 'white-bloom'],
    materials: ['resin'],
    estimatedTime: '约 8 分钟',
    checkOrder: '先排残液，再换清洗液，再减后固化',
    firstAction: '先检查清洗后有没有把凹槽和孔洞里的残液吹干。',
    relatedIds: ['resin-blooming', 'resin-wash-cure'],
    commonMisdiagnosis: ['把发雾误判成模型本身不透明'],
    searchAliases: ['发雾', '不透亮', '透明件发灰'],
  },
}

function inferPrinterType(problem) {
  if (problem.printerType) return problem.printerType
  const text = [problem.title, problem.subtitle, problem.description, ...(problem.causes || [])].join(' ')
  return /树脂|光固化|FEP|曝光|洗|固化|排液|中空件/.test(text) ? 'SLA' : 'FDM'
}

function inferStages(problem) {
  const text = [problem.title, problem.subtitle, problem.description, ...(problem.causes || [])].join(' ')

  if (/开始|不出丝|空打|起始|首层/.test(text)) return ['before']
  if (/热床|粘床|翘边|底层|第一层|象脚/.test(text)) return ['first-layer']
  if (/中途|一半|半路|层移|倒了|脱落|咔咔|刮/.test(text)) return ['mid-print']
  if (/清洗|固化|后处理|支撑|表面|顶面|尺寸|细节|层纹|发雾|泛白/.test(text)) return ['surface']

  return ['surface']
}

function inferMaterials(problem, printerType) {
  const text = [problem.title, problem.subtitle, problem.description, ...(problem.causes || [])].join(' ')

  if (printerType === 'SLA') return ['resin']

  const materials = []
  if (/PLA/.test(text)) materials.push('pla')
  if (/PETG/.test(text)) materials.push('petg')
  if (/ABS|ASA/.test(text)) materials.push('abs-asa')
  if (/TPU|柔性/.test(text)) materials.push('tpu')

  return materials.length ? [...new Set(materials)] : ['any']
}

function inferSeverity(problem) {
  if (problem.difficulty === '紧急') return 'urgent'
  if (problem.difficulty === '需处理' || problem.difficulty === '中等') return 'attention'
  if (problem.difficulty === '进阶') return 'advanced'
  return 'common'
}

function enrichProblem(problem) {
  const meta = STRUCTURED_META_BY_ID[problem.id] || {}
  const printerType = meta.printerType || inferPrinterType(problem)
  const stages = meta.stages || inferStages(problem)
  const materials = meta.materials || inferMaterials(problem, printerType)

  return {
    ...problem,
    printerType,
    stages,
    severity: meta.severity || inferSeverity(problem),
    materials,
    symptomTags: [...new Set(meta.symptomTags || [])],
    estimatedTime: meta.estimatedTime || '约 5-10 分钟',
    checkOrder: meta.checkOrder || '先排除基础项，再检查参数和结构件',
    firstAction: meta.firstAction || (problem.solutions?.[0] ? `先${problem.solutions[0].title}` : '先做一个小范围验证'),
    relatedIds: meta.relatedIds || [],
    commonMisdiagnosis: meta.commonMisdiagnosis || [],
    searchAliases: meta.searchAliases || [],
  }
}

export const problems = baseProblems.map(enrichProblem)
